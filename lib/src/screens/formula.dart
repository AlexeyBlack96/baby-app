...
import 'package:insta_clone/src/models/formula.dart' as model;
// import 'package:insta_clone/src/customIcons/custom_icons.dart';
...
class Formula extends StatefulWidget {
  @override
  final Baby baby;
  Formula({Key key, @required this.baby}) : super(key: key);
  _FormulaState createState() => _FormulaState();
}

class _FormulaState extends State<Formula> with SingleTickerProviderStateMixin {

  DateTime _date = DateTime.now();
  final _formKey = GlobalKey<FormState>();
  model.Formula data = new model.Formula("","","",DateTime.now(),"");
  @override
  void initState() {
    super.initState();
...
Widget appBar() {
    final formulaProvider = Provider.of<CRUDFormulaModel>(context);
      ...
      title: Row(
        children: <Widget>[
          ...
          Padding(
            padding: EdgeInsets.only(left:14.0, top:0.0, right: 14.0),
            child: Text(
            "Formula", style: TextStyle(color: Colors.orange)
          ),
          ),
          SizedBox(width: 4.0),
          ...
          onPressed:()async{
            if(_formKey.currentState.validate()){
              _formKey.currentState.save();
                await formulaProvider.addFormula(data);
